...
<script setup lang="ts">
	...
	const props = defineProps({
		...
		ui: {
			type: Object as PropType<
				Partial<typeof config.value & typeof cardConfig>
			>,
			default: () => ({}),
		},
	});
	const colorLight = computed(() => {
		...
		// @ts-ignore
		return (
			colors[props.color]?.["500"] || colors[props.color] || props.color
		);
	});
	const colorDark = computed(() => {
		...
		// @ts-ignore
		return (
			colors[props.color]?.["400"] || colors[props.color] || props.color
		);
	});
