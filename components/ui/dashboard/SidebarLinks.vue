<template>
  <component :is="draggable && Container ? Container : 'ul'" :class="ui.wrapper" v-bind="{
    ...
    ...attrs
  }" @drag-start="isDragging = true" @drag-end="isDragging = false" @drop="onDrop" @touchend="fixActionRestriction">
      ...
      <component :is="link.children?.length ? Disclosure : 'div'" v-slot="slotProps"
        ...
        <component :is="link.children?.length ? DisclosureButton : 'div'" as="template">
          <UTooltip class="flex" :popper="{ placement: 'right' }" :prevent="!link.tooltip" :ui="ui.tooltip"
            ...
            <ULink v-slot="{ isActive }"
              ...
              <UIcon v-if="link.children?.length && link.collapsible !== false" :name="ui.trailingIcon.name"
        ...
        <Transition v-bind="ui.transition" @enter="onEnter" @after-enter="onAfterEnter" @before-leave="onBeforeLeave"
          ...
          <DisclosurePanel v-if="link.children?.length && (slotProps?.open || link.collapsible === false)" static
            as="template">
            <UiDashboardSidebarLinks :level="level + 1" :links="link.children" :draggable="link.draggable" :ui="ui"
...
import type { DashboardSidebarLink } from '@/types/dashboard'
